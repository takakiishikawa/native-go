...
export async function POST(request: NextRequest) {
  ...
  const prompt = `あなたは英会話コーチです。学習者の英語スピーチを4観点で100点満点で採点し、日本語のフィードバックを作成してください。

## 採点観点（各100点満点）
1. 語彙：多彩で文脈に合った語彙が使えているか
2. 文法：「${grammarName}」を含む文法の正確さ
3. 流暢さ：詰まらず自然なペースで話せているか
4. 発音：テキストから推測できる発音・リズムの自然さ

## コメントのフォーマット（厳守）
[GOOD]（1〜2文。具体的な良い表現や良かった点）
[GRAMMAR_BEFORE]（実際に使った文法パターン、または改善できる文法表現。英語のみ）
[GRAMMAR_AFTER]（より自然・正確な文法表現。英語のみ）
[PHRASE_BEFORE]（実際に使ったフレーズや言い回し。英語のみ）
[PHRASE_AFTER]（より自然なフレーズや言い回し。英語のみ）
[EXAMPLE1]（「${grammarName}」を自然に使った英語例文。30語以内）
[EXAMPLE2]（別のシチュエーションでの英語例文。30語以内）

## トーンのルール
- 冷静・建設的・前向きなトーンで統一する
- 「ぜひ挑戦してください」「素晴らしい」などの過剰な表現は使わない
- スピーチが空または極端に短い場合は[GOOD]に「声に出す練習を続けていきましょう」と書き、GRAMMAR_BEFORE/PHRASE_BEFOREには"-"を入れる
...
スピーチ：${speechText || "(no speech detected)"}`
  ...
  const safeScores = evaluation.scores.map((s) => Math.min(100, Math.max(0, Math.round(Number(s) || 0))))
  const safeTotal = safeScores.length > 0
  ...
  return NextResponse.json({ logId: log.id, scores: safeScores, total: safeTotal, comment: evaluation.comment })
}
