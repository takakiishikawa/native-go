...
export async function POST(request: NextRequest) {
  ...
  const grammarInProgress = grammars.filter((g) => g.play_count > 0 && g.play_count < 10).length
  const expressionDone = expressions.filter((e) => e.play_count >= 10).length
  const expressionInProgress = expressions.filter((e) => e.play_count > 0 && e.play_count < 10).length

  const recent7 = logs.slice(0, 7)
  const weeklyGrammar = recent7.reduce((s, l) => s + (l.grammar_done_count ?? 0), 0)
  const weeklyExpression = recent7.reduce((s, l) => s + (l.expression_done_count ?? 0), 0)
