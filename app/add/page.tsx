    ...
    <Card className="border-blue-200">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base text-blue-700">{item.name}</CardTitle>
        ...
        <div>
          ...
          <ul className="space-y-1">
            {item.examples.map((ex, i) => (
              <li key={i} className="text-muted-foreground pl-2 border-l-2 border-blue-200">
                {ex}
              </li>
            ))}
          </ul>
        </div>
        <p className="text-muted-foreground text-xs">場面: {item.usage_scene}</p>
      </CardContent>
    </Card>
  )
}
    ...
    try {
      ...
      if (savedGrammars.length > 0) {
        ...
        toast.promise(genPromise, {
          ...
          success: "スピーキング用画像の生成完了！スピーキングページで練習できます",
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="space-y-3">
        ...
        />
        <Button onClick={handleExtract} disabled={!text.trim() || loading} className="w-full">
          {loading ? (
            <>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              文法 {result.grammar.length}件・表現 {result.expressions.length}件 抽出
            </p>
